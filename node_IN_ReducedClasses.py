...
BWQ = np.array((88736, 24224, 7232, 2336, 584))

mat = spio.loadmat('data/IN/selectedLabels/X_val_original.mat', squeeze_me = True)                                                                                         
X_val_original = mat['X_val_original']

mat = spio.loadmat('data/IN/selectedLabels/X_val_high.mat', squeeze_me = True)                                                                                         
X_val_high = mat['X_val_high']

mat = spio.loadmat('data/IN/selectedLabels/X_val_medium.mat', squeeze_me = True)                                                                                         
X_val_medium = mat['X_val_medium']

mat = spio.loadmat('data/IN/selectedLabels/X_val_low.mat', squeeze_me = True)                                                                                         
X_val_low = mat['X_val_low']

mat = spio.loadmat('data/IN/selectedLabels/X_val_verylow.mat', squeeze_me = True)                                                                                         
...
##    currentNode_obj = BBInitialLoss(SSRN_classifier, root)
    ...
    for i in range(50):
        file_obj = open("IN_ReducedClasses.txt", "a")
        root = Node(-1, None, (i+1)*3600, 0, 200)
        make_tree(root)
        currentNode_obj = BBInitialLoss(SSRN_classifier, root, 5)
        finalNode_obj = BB0(SSRN_classifier, currentNode_obj)
        file_obj.write(" " + str((i+1)*3600)+ " " + str(finalNode_obj.positions)+ " " + str(testNode(SSRN_classifier, finalNode_obj.positions)) + str(numOfIteration) + "\n")
        global numOfIteration
        numOfIteration = 0
        file_obj.close()
