...
def SSRN_model_fn(features, labels, mode):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
	    ...
	    summary_hook = tf.train.SummarySaverHook(
		    save_steps  =128,
		    output_dir = "KSC_model_ReducedClasses/summary/train",
...
def scenario(All_features_, All_labels):
    ...
    for quality in range(5):
        cAA = np.zeros((512, 614, 176))
        cA = None
        
        if(quality != 0):
            for i in range(176):
                cA = pywt.wavedec2(All_features_[:,:,i], 'db1', level = quality)[0]
                cAA[:,:,i] = imresize(cA, (512, 614)).astype('float32')
            All_features = cAA
        data = All_features.reshape(512 * 614, 176)                                                                                                         
        data = preprocessing.scale(data.astype('float64'))
        All_features = data.reshape(512, 614, 176)
        pad_length = 3                                                                                                                                     
        pad_depth = 0                                                                                                                                     
        ...
        for L in  range(1,9):
            count = 0;
            All_features_inBlocks = np.zeros((512 * 614, 7, 7, 176))
            All_labels_inBlocks = np.zeros((512 * 614))

            for i in range(3, 512 + 3):
                for j in range(3, 614 + 3):
                    if All_labels[i - 3, j - 3] == L:
                        All_features_inBlocks[count, :, :, :] = All_features_padded[i - 3 : i + 4, j - 3 : j + 4, :].reshape((1, 7, 7, 176))
    ...
    spio.savemat('data/KSC/selectedLabels/y_train.mat', {"y_train" : y_train_original})
    spio.savemat('data/KSC/selectedLabels/y_val.mat', {"y_val" : y_val_original})

    spio.savemat('data/KSC/selectedLabels/X_train_original.mat', {"X_train_original" : X_train_original})
    spio.savemat('data/KSC/selectedLabels/X_train_high.mat', {"X_train_high" : X_train_high})
    spio.savemat('data/KSC/selectedLabels/X_train_medium.mat', {"X_train_medium" : X_train_medium})
    spio.savemat('data/KSC/selectedLabels/X_train_low.mat', {"X_train_low" : X_train_low})
    spio.savemat('data/KSC/selectedLabels/X_train_verylow.mat', {"X_train_verylow" : X_train_verylow})
    
    spio.savemat('data/KSC/selectedLabels/X_val_original.mat', {"X_val_original" : X_val_original})
    spio.savemat('data/KSC/selectedLabels/X_val_high.mat', {"X_val_high" : X_val_high})
    spio.savemat('data/KSC/selectedLabels/X_val_medium.mat', {"X_val_medium" : X_val_medium})
    spio.savemat('data/KSC/selectedLabels/X_val_low.mat', {"X_val_low" : X_val_low})
    spio.savemat('data/KSC/selectedLabels/X_val_verylow.mat', {"X_val_verylow" : X_val_verylow})
        

def makeTrainingSet(indices):
    mat = spio.loadmat('data/KSC/selectedLabels/X_train_original.mat', squeeze_me = True)                                                                                         
    X_train_original = mat['X_train_original']
    mat = spio.loadmat('data/KSC/selectedLabels/X_train_high.mat', squeeze_me = True)                                                                                         
    X_train_high = mat['X_train_high']
    mat = spio.loadmat('data/KSC/selectedLabels/X_train_medium.mat', squeeze_me = True)                                                                                         
    X_train_medium = mat['X_train_medium']
    mat = spio.loadmat('data/KSC/selectedLabels/X_train_low.mat', squeeze_me = True)                                                                                         
    X_train_low = mat['X_train_low']
    mat = spio.loadmat('data/KSC/selectedLabels/X_train_verylow.mat', squeeze_me = True)                                                                                         
    X_train_verylow = mat['X_train_verylow']
    
    mat = spio.loadmat('data/KSC/selectedLabels/y_train.mat', squeeze_me = True)                                                                                         
    y_train = mat['y_train']
    
    X_train = np.zeros(X_train_original.shape, dtype = np.float64)

    for j in range(X_train.shape[0]):
        quality = np.sort(np.random.randint(176, size = 5)) #quality = 5 means the band has not been picked
...
##    spio.savemat('data/KSC/selectedLabels/y_train.mat',{"y_train":y_train_final})
##    spio.savemat('data/KSC/selectedLabels/X_train.mat',{"X_train":X_train_final})
...
##                                                                                                                                                   
##    mat = spio.loadmat('datasets/KSC/KSC_gt.mat', squeeze_me=True)                                                                                
##    All_labels = mat['KSC_gt']
    
##    for i in range(All_labels.shape[0]):
##        for j in range(All_labels.shape[1]):
##            if((All_labels[i, j] == 5)or(All_labels[i, j] == 6)):
##                All_labels[i, j] = 4
##            elif(All_labels[i, j] == 7):
##                All_labels[i, j] = 5
##            elif((All_labels[i, j] == 8)or(All_labels[i, j] == 9)or(All_labels[i, j] == 10)or(All_labels[i, j] == 11)):
##                All_labels[i, j] = 6
##            elif(All_labels[i, j] == 12):
##                All_labels[i, j] = 7
...
##    mat = spio.loadmat('data/KSC/selectedLabels/X_train.mat', squeeze_me = True)                                                                                         
##    X_train = mat['X_train']
##    mat = spio.loadmat('data/KSC/selectedLabels/y_train.mat', squeeze_me = True)                                                                                         
##    y_train = mat['y_train']
