...
def bandSel(C, n, m, numOfIter, X_test, y_test): 
    # marray = np.zeros(p)
    p = 4
    finalacc = 0
    finalresult = None
    data = np.copy(X_test)
    GT = y_test
    testSetSize = int(data.shape[0])
    bands = np.zeros(m)
    d = np.zeros(n-1)
    dd = np.zeros(p)
    nnb = np.zeros([testSetSize, p])
    nb = np.zeros(p);
    s = n // p
    acc = 0
    allIndex = 0
    
##    print("*Inside bandSel:: test set shape is: ", data.shape)

    for k in range(testSetSize):
        extra = 0
        for i in range(n-1):
            d[i] = abs(data[k,3,3, i+1] - data[k,3,3, i])
            
        for j in range(p):
            dd[j] = 1/s * np.sum(d[j*s:(j+1)*s])
        sumOfDD = np.sum(dd)

        for j in range(p):
            nnb[k,j] = dd[j]*m/sumOfDD
            if((nnb[k,j]<s) and (extra>0)):
                nnb[k,j] += extra
                extra = 0
            if(nnb[k,j]>s):
                extra += nnb[k,j] - s
...
##                print("i is: ", i, " nb[j] is: ", nb[j])
                if(i >= nb[j]):
                    data[:, :, :, j*s + value] = -2
                if(i < nb[j]):
                    if (np.all(allIndex == 0)):
                        allIndex = j*s + value
                    else:
                        allIndex = np.append(allIndex, j*s + value)
##            print("^^^^%%## allIndex is: ", allIndex)
        print("nb is: ")
        print(nb)
        print("Step: ", i, "out of ", numOfIter)
        test_input_fn = tf.estimator.inputs.numpy_input_fn(
            ...
            num_epochs = 1,
            shuffle = False)
##        print("The test set type is: ", test_input_fn(self, 'x'))
        results = C.evaluate(input_fn = test_input_fn, hooks = None)
##        print("********Sup?*******")
        acc = results['accuracy']
        if(acc>=finalacc):
            finalacc = acc
            finalresult = results
        print("The result of this iteration is: ", acc)
        print("The final result is: ", finalacc)
        
    print("The final accuracy is: ", finalacc)
    return (finalresult, allIndex)

def main():
    mat = spio.loadmat('data/KSC/allLabels/X_test_original.mat', squeeze_me = True)                                                                                         
    X_val_original = mat['X_test_original']
...
##            shuffle = False)
####        print("The test set type is: ", test_input_fn(self, 'x'))
##    results = SSRN_classifier.evaluate(input_fn = test_input_fn, hooks = None)
##    bandSel(SSRN_classifier, datasetBands ,175 , numOfIter, X_val_original, y_val)
    for i in range(steps):
        print("#########step: ", (i+2))
        file_obj = open("KSC_BSCNN.txt", "a")
        (finalresult, allIndex) = bandSel(SSRN_classifier, datasetBands ,(i+2) , numOfIter, X_val_original, y_val)
        file_obj.write(" " + str((i+2))+ " " + str(finalresult["accuracy"]) + "\n")
        file_obj.close()
##    file_obj = open("UP_BSCNN.txt", "a")
##    (finalresult, allIndex) = bandSel(SSRN_classifier, datasetBands ,12 , numOfIter, X_val_original, y_val)
##    file_obj.write(" " + str(12)+ " " + str(finalresult["accuracy"]) + "\n")
##    file_obj.close()
##    data = np.zeros(X_val_original.shape)
##    test_input_fn = tf.estimator.inputs.numpy_input_fn(
##            x = {"x": data},
##            y = y_val,
##            batch_size = 16,
